...
def select_image():  # function to select an image
    ...
    if len(detection) == 0:  # if the text is not found
        ...
        photo1 = ImageTk.PhotoImage(original_image)  # convert the image to PhotoImage format
        ...
        photo2 = ImageTk.PhotoImage(image)  # convert the image to PhotoImage format

        plate = cv2.imread("placeholder.jpg")  # read the image
        ...
        photo3 = ImageTk.PhotoImage(plate)  # convert the image to PhotoImage format

        # Update image labels in the GUI
        label1.configure(image=photo1)  # update the image label
        label1.image = photo1  # keep a reference

        label2.configure(image=photo2)  # update the image label
        label2.image = photo2  # keep a reference

        label3.configure(image=photo3)  # update the image label
